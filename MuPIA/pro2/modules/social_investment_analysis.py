...
class Social():   
    ...
    def calculate_residual_value(self):
        for year in range(self.analysis_period):
            if year == self.analysis_period -1:
                self.residual_value.append((2*self.measure['lifetime']-self.analysis_period)*self.measure['cost']/self.measure['lifetime'])
            else: 
    ...
    def calculate_equipment_cost(self):
        for year in range(self.analysis_period):
            if year == 0:
                self.equipment_cost.append(self.measure['cost'])
